...
# MAGIC 5. Export YugabyteDB table into Delta Lake Table using YugabyteDB Smart driver with follower read option
...
# MAGIC         .format("jdbc") \
# MAGIC         .option("url", "jdbc:yugabytedb://10.14.16.7:5433/yugabyte") \
# MAGIC         .option("dbtable", "public.employee") \
# MAGIC         .option("user", "yugabyte") \
# MAGIC         .option("driver", "com.yugabyte.Driver") \
# MAGIC         .option("load-balance", "true") \
# MAGIC         .option("sessionInitStatement","set session characteristics as transaction read only;set yb_read_from_followers = true;")\
# MAGIC         .option("password", "xxxx") \
# MAGIC         .option("useSSL", "true") \
# MAGIC         .option("ssl", "true") \
# MAGIC         .option("sslmode", "require") \
# MAGIC         .load()
# MAGIC jdbcDF.write.format ( "delta" ).mode("overwrite").saveAsTable ( "Employee" )
